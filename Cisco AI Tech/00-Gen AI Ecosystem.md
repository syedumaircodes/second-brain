...
You have likely heard that a large language model, or LLM, thinks. But its method of thinking is fundamentally different from our own. To use these powerful tools effectively, we must understand their core mechanism.

At its core, an LLM is a sophisticated prediction engine. It does not understand stories in a human sense. Instead, it analyzes input and calculates the most probable next word based on the patterns in its training data.

You begin with a prompt, telling an AI to write a story, and you give it a starting phrase like "once upon a--." The LLM assesses this common phrase. Based on analyzing countless books, articles, and websites, its training data shows an overwhelmingly high probability that the next word is "time." Other words, like "kingdom" or "land" are possible, but statistically far less likely to immediately follow.

The model selects the word with the highest probability and adds it to the sequence. Think of this process as the world's most advanced autocomplete, capable of building entire narratives. The LLM has now formed the beginning of a sentence.

The process repeats. This new phrase is fed back into the model to predict what comes next. Given the context, a word like "there" or "in" becomes highly probable, continuing the narrative logic. The model adds the most likely word, and the loop continues.

By repeatedly predicting the next most likely word, an LLM can generate everything from simple phrases to complex code and detailed stories. Understanding that an LLM's output is based on probability and not true knowledge is the key to using AI effectively and recognizing its strategic limitations.

---
## Generative AI Models and Outputs
Your assignment from Maria is to provide a practical assessment of Generative AI, but you realize that to evaluate the tools, you first need to understand the engines that power them. It's not enough to know the name "ChatGPT"; you need to grasp what kind of model it is and how it differs from an image-generation tool like Midjourney.|

This topic provides the foundational vocabulary to distinguish between the core model types that drive the majority of today's Generative AI applications.

## AI Models: The Core Engine

A "model" in artificial intelligence is a trained algorithm that uses large datasets to perform specific tasks. It is a mathematical system – often a neural network – that learns data patterns. The model acts as the "brain," while AI tools, such as chatbots, serve as interfaces to it. One model can support multiple tools; for example, OpenAI's GPT-4 powers ChatGPT and other custom applications. The model is the core source of an AI system’s capabilities.

## Large Language Models: The Engines of Text

The most common type of model you will encounter is the Large Language Model, or LLM. LLMs are a category of deep learning models trained on immense amounts of text data – essentially, a significant portion of the public internet, books, articles, and other sources. Their fundamental skill, as shown in the previous video, is predicting the next most probable word in a sequence. While this sounds simple, at a massive scale, this capability allows LLMs to perform a wide range of sophisticated natural language tasks, as shown in the figure below.

[![A diagram showing the L L M Engine. A central icon labeled Large Language Model has arrows pointing out to various output document icons labeled Answering Questions, Summarization, Translation, Content Creation, Code Generation.](https://ondemandelearning.cisco.com/apollo-alpha/ai-tech-genaiecosystem-10/assets/Artificial%20Intelligence/Modular/AITECH/v1.0/AITECH_C11/AITECH-10-11-LLMs-00.svg "A diagram showing the L L M Engine. A central icon labeled Large Language Model has arrows pointing out to various output document icons labeled Answering Questions, Summarization, Translation, Content Creation, Code Generation.")](https://ondemandelearning.cisco.com/apollo-alpha/ai-tech-genaiecosystem-10/assets/Artificial%20Intelligence/Modular/AITECH/v1.0/AITECH_C11/AITECH-10-11-LLMs-00.svg)

As shown in the diagram, these are some of the most common outputs from LLMs:

- **Answering Questions:** Responding to factual queries
    
- **Summarization:** Condensing long documents into key points
    
- **Translation:** Translating text between different languages
    
- **Content Creation:** Drafting emails, reports, technical documentation, and articles
